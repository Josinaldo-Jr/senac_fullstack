...
let tempo = 15; // Tempo em segundos
...
function verificarFimDeJogo() {
    if (tentativasRestantes === 0) {
        ...
        salvarPalavraErrada(letrasIncorretas);
        reiniciarJogo();
...
function carregarPalavrasErradas() {
    return JSON.parse(localStorage.getItem("palavrasErradas")) || [];
}

function salvarPalavraErrada(palavra) {
    const palavrasErradas = JSON.parse(localStorage.getItem("palavrasErradas")) || [];
    palavrasErradas.push(palavra);
    localStorage.setItem("palavrasErradas", JSON.stringify(palavrasErradas));
}
...
const palavrasErradas = carregarPalavrasErradas();
console.log("Palavras erradas armazenadas:", palavrasErradas);
